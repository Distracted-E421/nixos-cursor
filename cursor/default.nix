...
# To update: cd cursor && ./update.sh
{
  ...
  # IMPORTANT: Always provide srcUrl! The fallback downloader.cursor.sh is DEPRECATED and may not work.
  # Use downloads.cursor.com URLs from cursor-versions.nix instead.
  srcUrl ? null, # x86_64 download URL (REQUIRED - downloader.cursor.sh fallback is broken)
  srcUrlAarch64 ? null, # aarch64 download URL (null = derive from srcUrl)
  ...
  argsList =
    if lib.isList commandLineArgs
    then commandLineArgs
    else lib.splitString " " commandLineArgs;
  ...
  effectiveSrcUrlAarch64 =
    if srcUrlAarch64 != null
    then srcUrlAarch64
    else deriveAarch64Url srcUrl;

  # Select source: local file > specific URL > standard URL
  appImageSrc =
    if localAppImage != null
    then
      # Use local AppImage (for DNS issues or offline builds)
      localAppImage
    else if hostPlatform.system == "x86_64-linux" && srcUrl != null
    then
    ...
    else if hostPlatform.system == "aarch64-linux" && effectiveSrcUrlAarch64 != null
    then
